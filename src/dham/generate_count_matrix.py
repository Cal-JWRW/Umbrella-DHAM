...
def gcm(disc_data, nbins, lagt):
    ...
    for i in range(ntraj):
        ...
        for j in range(lagt, nsteps):

            # State at time t
            current_state = disc_data[i, j-lagt]

            # State at time t + lag
            transition_state = disc_data[i, j]

            # Increment the transition count for this trajectory
            nCm[i, current_state, transition_state] += 1
    ...
    for i in range(ntraj):
        for j in range(nbins):
            ...
            for k in range(nbins):
                transition_matrix[j, k] += nCm[i, j, k]

            # Count total outgoing transitions from state j in trajectory i
            transition_total[i, j] = np.sum(nCm[i, j, :])
